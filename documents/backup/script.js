/**
 * @file WebSocket client and UI bindings for real-time status, temperature, humidity,
 * and setpoint control in the Gas Log Controller web UI.
 */
...
function updateStatusDisplay(statusMessage) {
  ...
  const flame = document.getElementById('flame-indicator');
  if (flame) {
    if (statusMessage.includes('ON')) {
      flame.className = 'flame-on';
    } else {
      flame.className = 'flame-off';
    }
  }
}
...
if (slider && setpointDisplay) {
  slider.addEventListener("input", () => {
    ...
    // Send setpoint change via WebSocket
    const message = JSON.stringify({
      type: "setpoint",
